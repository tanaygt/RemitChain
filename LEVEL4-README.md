...
- Uses `vault::Client` generated by `contractimport!` for type-safe inter-contract calls.
- The `deposit_fees` call is atomic and occurs within the same transaction as the remittance creation.
...
- Payment asset: Official Stellar Testnet USDC (`GBBD47IF...A5`).
- Fee accounting: Managed via a dedicated, standalone `FeeVault` contract.
- Implementation: Uses the Stellar Asset Contract (SAC) standard for all token transfers.
